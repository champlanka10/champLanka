...
export function WhatsAppButton({ message }: { message?: string }) {
  ...
  return (
    <a
      href={whatsappLink(message ?? "Hi Champ Lanka, I'd like to plan a trip to Sri Lanka.")}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Chat with us on WhatsApp"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      className="fixed bottom-5 right-5 z-50 flex items-center gap-3 rounded-full bg-whatsapp pl-4 pr-5 py-3 text-whatsapp-foreground shadow-elegant transition-bounce hover:scale-105 sm:bottom-6 sm:right-6"
    >
      <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white/15">
        <WhatsAppGlyph className="h-6 w-6" />
      </span>
      <span className={cn("hidden text-left text-sm font-semibold leading-tight sm:block", hovered && "text-white")}>
        WhatsApp
        <span className="block text-[11px] font-normal text-white/85">Chat with us</span>
      </span>
    </a>
  );
}
